 ...
 */
class FkOrderController extends BaseController
{
    /**
     * @inheritDoc
     */
            ...
            if ($valid) {
                ...
                try {
                    if ($flag = $model->save(false)) {
                        if (! empty($deletedIDs)) {
                            FkOrderProduct::deleteAll(['id' => $deletedIDs]);
                        }
    ...
    public function actionPriceList($key,$id)
    {
        ...
        if (!empty($product)) {
            ...
            foreach($prices as $value=>$price) {
                echo '<label><input type="radio" name="FkOrderProduct['.$key.'][price_number]" value="'.$value.'"> '.$price.'</label>';
            }
        }
    }
    ...
    {
        if (($model = FkOrder::findOne(['id' => $id])) !== null) {
            return $model;
        }
    ...
    public function actionCreate()
    {
        ...
        $model = new FkOrder();
        $model_product = [new FkOrderProduct()];
        if ($model->load($post)) {

            $model_product = Model::createMultiple(FkOrderProduct::classname());
            Model::loadMultiple($model_product, $post);
    ...
    public function actionOrderProducts()
    {
        ...
        if (isset($_POST['depdrop_parents'])) {
            ...
            if ($parents != null) {
                ...
                $OrderProducts = FkOrderProduct::find()->where(['order_id'=>$order_id])->all();
    ...
    {
        $order = FkOrder::findOne($order_id);
        $orderProduct = FkOrderProduct::find()->where(['order_id'=>$order_id,'product_id'=>$product_id])->one();
