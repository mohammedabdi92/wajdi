 ...
 */
class FkOrderProduct extends \common\components\BaseModel
{
    ...
    public function rules()
    {
        return [
            ...
            [['count'], 'number'],
            [['count_type_name','clone_by','clone_at'], 'safe'],
    ...
    public function checkDuplicate($attr, $params) {
        ...
        if (isset($this->product_id)) {
            $item_order = FkOrderProduct::find()->where(['order_id'=>$this->order_id,'product_id'=>$this->product_id])->andWhere(['<>','id',$this->id??0])->one() ;
    ...
    {
        return $this->hasOne(FkOrder::className(), ['id' => 'order_id']);
    }
